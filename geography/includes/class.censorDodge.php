...
class censorDodge {
    public $version = "1.82 BETA - TitaniumNetwork Edition";
    ...
    private $unsupportedWebsites = array("youtube.com", "surviv.io", "agar.io", "krunker.io", "shellshock.io");
    ...
    public $customUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:60.0) Gecko/20100101 Firefox/60.0";
    public $customReferrer = null;

    //Additional settings that are applied for the page request
    public $curlSettings = array();

    function __construct($URL = "", $logToFile = false, $preventHotlinking = false) {
            ...
            foreach ((array)$website as $u) {
                if (!empty($u) && is_string($u)) { //Check that the URL is valid
                    ...
                    if (preg_match('~('.$u.')~i', $this->URL, $d)) { //Check if URL is permitted, and send an error to the stop script
                        ...
                        throw new Exception("Access to ".$d[0]." is not permitted.");
                    }
                }
            }
...
//unsupported soots
    public function unsupportedWebsite($website) {
        if (!empty($website)) {
            foreach ((array)$website as $u) {
                if (!empty($u) && is_string($u)) { //Check that the URL is valid
                    $this->unsupportedWebsites[] = $u; //Add individual URL to the array

                    if (preg_match('~('.$u.')~i', $this->URL, $d)) { //Check if URL is permitted, and send an error to the stop script
                        if (empty($d[0])) { $d[0] = parse_url(trim($this->URL), PHP_URL_HOST); }
                        throw new Exception($d[0]." is currently not supported by Titanium CensorDodge.");
                    }
                }
            }
    ...
    public function openPage() {
		...
		if (@$_SESSION[$hl]!=true && $h!=parse_url(cdURL, PHP_URL_HOST)) { throw new Exception("The use of hotlinking is strictly forbidden on this server!"); } else { $_SESSION[$hl] = true; } if (!$i) { session_destroy(); } else { session_write_close(); } }
            ...
            if ($this->unsupportedWebsites && preg_match('~('.implode("|", $this->unsupportedWebsites).')~', $this->URL, $d)) { throw new Exception("Sorry, but ".$d[0]." is currently not supported on Titanium CensorDodge, please try using Node Unblocker."); }
